...
vi.mock('./ResearchTimeSeriesChart', () => ({
  ResearchTimeSeriesChart: (props: { file: string; initialTransformation: string; zeroLine?: boolean }) => {
    chart(props)
    return <div data-testid="research-chart">{props.file} · {props.initialTransformation} · {props.zeroLine ? 'zero line' : 'no zero line'}</div>
  },
}))
...
describe('LiquiditySignalChart', () => {
  ...
  it('switches between actual, standardized, and locked-model residual views', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Actual YoY' }))
    expect(screen.getByTestId('research-chart')).toHaveTextContent('gm2-oil-lead.json · raw')

    fireEvent.click(screen.getByRole('button', { name: 'Residuals (GM2 removed)' }))
