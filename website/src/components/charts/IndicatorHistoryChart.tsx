  ...
  return <section aria-label={`Full history of ${indicator.label}`}>
    ...
    <div className="mt-5 h-[390px] w-full" tabIndex={0} aria-label={`${indicator.label} interactive historical line chart`}>
      <ResponsiveContainer width="100%" height="100%"><LineChart data={rows} margin={{ top: 10, right: 14, left: 8, bottom: 8 }} onClick={(next: unknown) => { const point = next as { activeLabel?: string }; if (point.activeLabel) setLockedDate(point.activeLabel) }}>
        <CartesianGrid opacity={0.25} /><XAxis dataKey="date" minTickGap={40} tickFormatter={(date) => String(date).slice(0, 7)} fontSize={11} /><YAxis width={64} fontSize={11} label={{ value: indicator.unit, angle: -90, position: 'insideLeft', fontSize: 10 }} />
        {indicator.referenceRanges.p25 !== null && indicator.referenceRanges.p75 !== null && <ReferenceArea y1={indicator.referenceRanges.p25} y2={indicator.referenceRanges.p75} fill="#0f766e" fillOpacity={0.08} />}
        {indicator.referenceRanges.historicalMedian !== null && <ReferenceLine y={indicator.referenceRanges.historicalMedian} stroke="#78716c" strokeDasharray="4 3" label={{ value: 'Historical median', fontSize: 10 }} />}
        ...
        <Line type="monotone" dataKey="value" name={indicator.label} stroke="#0f766e" strokeWidth={2} dot={false} connectNulls={false} isAnimationActive={false} />
        {rows.length > 36 && <Brush dataKey="date" height={23} travellerWidth={9} stroke="#0f766e" />}
