...
const colors = ['#0f766e', '#2563eb', '#d97706', '#be123c', '#7c3aed', '#475569', '#15803d', '#a16207']
...
export function LayerHistoryChart({ indicators }: { indicators: IndicatorDataset[] }) {
  ...
  return <div className="border-y border-stone-200 py-5 dark:border-stone-800">
    <div className="flex flex-wrap gap-2">{indicators.map((indicator, index) => <button key={indicator.id} type="button" onClick={() => setVisible((current) => current.includes(indicator.id) ? current.filter((id) => id !== indicator.id) : [...current, indicator.id])} className={`border px-3 py-2 text-xs font-semibold ${visible.includes(indicator.id) ? 'text-white' : 'border-stone-300 dark:border-stone-700'}`} style={visible.includes(indicator.id) ? { backgroundColor: colors[index % colors.length], borderColor: colors[index % colors.length] } : undefined}>{indicator.label}</button>)}</div>
    <div className="mt-4 h-64"><ResponsiveContainer><LineChart data={rows}><CartesianGrid opacity={0.2} /><XAxis dataKey="date" minTickGap={48} tickFormatter={(date) => String(date).slice(0, 4)} fontSize={11} /><YAxis width={52} label={{ value: 'z-score', angle: -90, position: 'insideLeft', fontSize: 10 }} /><Tooltip labelFormatter={(date) => String(date).slice(0, 7)} formatter={(value, name) => [Number(value).toFixed(2), indicators.find((item) => item.id === name)?.label ?? name]} />{indicators.filter((item) => visible.includes(item.id)).map((indicator, index) => <Line key={indicator.id} dataKey={indicator.id} stroke={colors[indicators.indexOf(indicator) % colors.length]} strokeWidth={1.7} dot={false} connectNulls={false} isAnimationActive={false} />)}</LineChart></ResponsiveContainer></div>
