...
export function ConceptExplainerChart({ concept }: { concept: ConceptType }) {
  ...
  return <div className="border border-stone-200 p-4 dark:border-stone-800">
    ...
    <div className="mt-3 h-44"><ResponsiveContainer><LineChart data={data}><CartesianGrid opacity={0.2} /><XAxis dataKey="month" /><YAxis domain={['auto', 'auto']} /><Tooltip />{concept === 'zscore' && <ReferenceLine y={mean} label="Mean" stroke="#78716c" strokeDasharray="4 3" />}{concept === 'yoy' && <><ReferenceLine x="M1" stroke="#d97706" /><ReferenceLine x="M13" stroke="#0f766e" /></>}<Line dataKey={concept === 'indexed' ? 'indexed' : 'value'} stroke="#0f766e" strokeWidth={2} dot /></LineChart></ResponsiveContainer></div>
