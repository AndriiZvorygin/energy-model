...
export function CurrentStateIndicatorCard({ indicator, onExpand, showEvidenceContext = false }: { indicator: IndicatorDataset; onExpand: () => void; showEvidenceContext?: boolean }) {
  ...
  return <article className="cursor-pointer border border-stone-200 bg-white p-5 transition hover:border-petroleum/60 dark:border-stone-800 dark:bg-[#18201d]" onClick={onExpand} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onExpand() }} role="button" tabIndex={0} aria-label={`Open full history for ${indicator.label}`}>
    <div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold text-ink dark:text-white">{indicator.label}</h3><p className="mt-1 text-xs text-stone-500">Observation {indicator.latest.date.slice(0, 7)}{indicator.latest.sourceDate && indicator.latest.sourceDate !== indicator.latest.date ? ` · source release ${indicator.latest.sourceDate.slice(0, 10)}` : ''} · {indicator.frequency} · {freshness}</p></div><EvidenceLabel label={indicator.evidenceLabel} /></div>
    ...
    {showEvidenceContext && <div className="mt-4 grid gap-2 border-t border-stone-200 pt-3 text-xs leading-5 text-stone-500 dark:border-stone-800"><p><strong className="text-ink dark:text-white">Confirm with:</strong> {indicator.confirmingIndicators.join('; ') || 'No linked indicator documented'}</p><p><strong className="text-ink dark:text-white">Could conflict:</strong> {indicator.conflictingIndicators.join('; ') || 'No linked indicator documented'}</p></div>}
