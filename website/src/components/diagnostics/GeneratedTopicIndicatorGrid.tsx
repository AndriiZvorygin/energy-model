...
type SummaryRow = { indicatorFile: string | null }
...
export function GeneratedTopicIndicatorGrid() {
  ...
  const files = useMemo(() => {
    ...
    if (!summary) return []
    return [...new Set(['supporting', 'mixed', 'contradicting'].flatMap((status) => summary[status as keyof Summary]).map((row) => row.indicatorFile).filter((file): file is string => Boolean(file)))]
  }, [evidence, route])
