...
import type { CurrentClassification, GeneratedManifest, IndicatorDataset } from '../components/charts/chartTypes'
...
export function CurrentState() {
  ...
  const { data: classification, error: classificationError } = useGeneratedJson<CurrentClassification>('current-classification.json')
  ...
  if (manifestError || error || classificationError || schemaError) return <><PageHeader eyebrow="Latest observations" title="Current state by evidence layer" description="Historical indicator data could not be loaded." /><PageBody><p className="text-sm text-amber-700">{manifestError ?? error ?? classificationError ?? schemaError}</p></PageBody></>
  return <><PageHeader eyebrow="Latest observations" title="Current state in historical context" description="Every reading is placed against its own history, source date, frequency, confirming evidence, and conflicts. There is no aggregate red or green verdict." /><PageBody>
    {!manifest || !indicators.length || !classification ? <div className="flex h-64 items-center justify-center text-sm text-stone-500">Loading indicator histories…</div> : <>
      <ClassificationSummary classification={classification} compact />
      <div className="mt-8"><GeneratedEvidenceSummary topic="current_state_us" /></div>
      <section className="mt-8 grid gap-6 border-b border-stone-300 pb-8 md:grid-cols-3 dark:border-stone-700"><div><h3 className="text-sm font-semibold">Active symptoms</h3><p className="mt-2 text-sm text-stone-500">{classification.activeSymptoms.map((item) => item.name).join(', ') || 'None meet active persistence rules.'}</p></div><div><h3 className="text-sm font-semibold">Emerging symptoms</h3><p className="mt-2 text-sm text-stone-500">{classification.emergingSymptoms.map((item) => item.name).join(', ') || 'None currently emerging.'}</p></div><div><h3 className="text-sm font-semibold">Strongest candidate evidence</h3><p className="mt-2 text-sm text-stone-500">{classification.supportingIndicators.slice(0, 3).map((item) => item.label).join(', ') || 'No indicators clear the strong-support threshold.'}</p></div></section>
