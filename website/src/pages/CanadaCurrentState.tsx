...
type CanadaManifest = {
  ...
  indicators: Array<{ id: string; file: string; label: string; geography: string; layer: string; core: boolean; latestDate: string }>
}
type CanadaState = { status: string; latestObservationDate: string; notes: string[] }

const layerOrder = ['Global oil and liquidity inputs', 'Physical energy conditions', 'Energy affordability and finance', 'Production and output', 'Labour and households']
  ...
  return <><PageHeader eyebrow="Canadian-centred research" title={title} description="Canadian evidence is shown with its own history, definitions and source dates. No Canadian symptom or regime classifier is calculated in this release." /><PageBody>
    ...
    {!manifest || !state || !indicators.length ? <div className="flex h-64 items-center justify-center text-sm text-stone-500">Loading Canadian indicator histories…</div> : <>
      ...
      <div className="mt-12 space-y-16">{visibleLayers.map((layerName) => {
        ...
        return <section key={layerName}><div className="border-b border-stone-300 pb-4 dark:border-stone-700"><p className="text-xs font-semibold uppercase text-petroleum">{geography === 'Ontario' && layerName !== 'Global oil and liquidity inputs' ? 'Ontario regional context' : 'Evidence layer'}</p><h2 className="mt-2 text-2xl font-semibold">{layerName}</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-stone-500">{layerText[layerName]}</p></div><div className="mt-6"><LayerHistoryChart indicators={rows} /></div><div className="mt-6 grid gap-4 xl:grid-cols-2">{rows.map((indicator) => <CurrentStateIndicatorCard key={indicator.id} indicator={indicator} onExpand={() => setSelected(indicator)} showEvidenceContext />)}</div></section>
      })}</div>
    </>}
    <ChartModal open={Boolean(selected)} title={selected?.label ?? 'Canadian indicator history'} onClose={() => setSelected(null)}>{selected && <IndicatorHistoryChart indicator={selected} />}</ChartModal>
  </PageBody></>
}

export function CanadaCurrentState() { return <CanadaStatePage /> }
export function CanadaEnergy() { return <CanadaStatePage layer="Physical energy conditions" /> }
export function CanadaEconomy() { return <CanadaStatePage layer="Production and output" /> }
export function CanadaLabour() { return <CanadaStatePage layer="Labour and households" /> }
export function CanadaHouseholds() { return <CanadaStatePage layer="Energy affordability and finance" /> }
export function CanadaOntario() { return <CanadaStatePage geography="Ontario" /> }
