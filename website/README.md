...
Current State, Regimes, Symptoms, Canada, Food, Housing, and Affordability pages place a compact interpretation and evidence matrix before the detailed charts. The Python refinery generates these matrices through one `generate_evidence_summary(geography, topic)` interface and stores them under canonical keys such as `canada:current-state`. `presentation-manifest.json` maps each route to explicit geography and topic fields; website components only render the published contract. Selecting a count filters the matrix, and selecting a row reveals its values, source date, calculation, limitations, sparkline, historical range, and full interactive history when available.
